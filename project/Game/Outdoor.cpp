...

{
    ...
    if (e->type == SDL_MOUSEBUTTONDOWN) ///to click & enter house
    {
        int x = e->button.x;
        int y = e->button.y;
...
void Outdoor::GetHouseEntrance()
{
    ...
    for(int i = 0; i<4; i++ )
    {
        ...
        for(int x = 0; x<n; x++ )
        {
            entrance[countEntrances] = entry[x];
    ...
    for (int i = countEntrances; i < 12; i++) // to point remaining entrance pointers to null
    {
        entrance[i] = NULL;
    }
}
